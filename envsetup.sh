...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^chidori_") ; then
        CHIDORI_BUILD=$(echo -n $1 | sed -e 's/^chidori_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $CHIDORI_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        CHIDORI_BUILD=
    fi
    export CHIDORI_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${CHIDORI_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    CHIDORI_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/chidori/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the CHIDORI model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch chidori_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Chidori GitHub
        T=$(gettop)
        cd $T > /dev/null
...
function repopick() {
    T=$(gettop)
    $T/vendor/chidori/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $CHIDORI_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.chidori.device | grep -q "$CHIDORI_BUILD");
    then
        ...
        echo "The connected device does not appear to be $CHIDORI_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.chidori.device | grep -q "$CHIDORI_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
function make()
{
    if [ "$GENERATE_HIDL" == "true" ]; then
	    ...
	    source "$(gettop)"/tools/vendor_hal_makefile_generator.sh
	    RET=$?
	    if [ $RET -ne 0 ]; then
...
. $ANDROID_BUILD_TOP/vendor/chidori/build/envsetup.sh
